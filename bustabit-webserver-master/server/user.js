...
exports.register  = function(req, res, next) {
	...
	var nameid = lib.removeNullsAndTrim(values.user.nameid);
   ...
   /* var email = lib.removeNullsAndTrim(values.user.email);*/
	//add by lt
	var solecodeinput = lib.removeNullsAndTrim(values.user.sub_partner_id);
    var phone = lib.removeNullsAndTrim(values.user.phone);
    var depositname = lib.removeNullsAndTrim(values.user.deposit_name);
    var bankIndex = lib.removeNullsAndTrim(values.user.bank);
	var accountnum = lib.removeNullsAndTrim(values.user.account_num);
  ...
  //  database.createUser(username, password, email, ipAddress, userAgent, function(err, sessionId) {
	  database.createUser(username, nameid, password, solecodeinput, phone, depositname, bankIndex, accountnum, ipAddress, userAgent, addDomain,  function(err, sessionId) {
        if (err) {
            if (err === 'USERNAME_TAKEN') {
                values.user.name = null;
                return res.render('register', { warning: '사용자 계정이 중복되었습니다.', values: values.user});
            }
            return next(new Error('Unable to register user: \n' + err));
        }
        //res.cookie('id', sessionId, sessionOptions);//승인된 계정만 로그인*****
        return res.redirect('/');
    });
};
...
exports.login = function(req, res, next) {
    ...
    if (!username || !password)
        return res.render('login', { warning: '' });

    database.validateUser(username, password, otp, ipAddress, function(err, userId) {
        if (err) {
            console.log('[Login] Error for ', username, ' err: ', err);
			if (err === 'NO_IP')
                return res.render('login',{ warning: '차단된 아이피 입니다.' });
            if (err === 'NO_USER')
                return res.render('login',{ warning: '존재하지않는 계정 입니다.' });
			if (err === 'NO_PLAY')
                return res.render('login',{ warning: '승인되지 않은 계정입니다.' });
			if (err === 'NO_LOGIN')
                return res.render('login',{ warning: '강제퇴장된 계정입니다.' });
            if (err === 'WRONG_PASSWORD')
                return res.render('login', { warning: '비밀번호가 틀렸습니다.' });
            if (err === 'INVALID_OTP') {
        ...
        assert(userId);
		database.validateSession(username, function(err, length){
			if (err) {
			...
			database.createSession(userId, ipAddress, userAgent, remember, function(err, sessionId, expires) {
				...
				database.setlogcnt(userId, function(err) {
					if (err) {
						return next(new Error('Unable to get deposits: \n' + err));
					}
...
exports.adminlogin = function(req, res, next) {
	...
	*/
    ...
    if (!username || !password)
        return res.render('adminlogin', { warning: 'no username or password' });

    database.validateAdmin(username, password, otp, function(err, userId) {
        if (err) {
            console.log('[Login] Error for ', username, ' err: ', err);

            if (err === 'NO_USER')
                return res.render('adminlogin',{ warning: 'Username does not exist' });
            if (err === 'WRONG_PASSWORD')
                return res.render('adminlogin', { warning: 'Invalid password' });
            if (err === 'INVALID_OTP') {
                var warning = otp ? 'Invalid one-time password' : undefined;
                return res.render('login-mfa', { username: username, password: password, warning: warning });
            }
...
exports.profile = function(req, res, next) {
    ...
    database.getPublicStats(username, function(err, stats) {
		...
		if(stats.games_played > 0 && pages == 0)
		{
			pages = 1;
		}
...
exports.resetPassword = function(req, res, next) {
    ...
    database.validateUser(user.username, password, otp, ipAddress, function(err, userId) {
        if (err) {
