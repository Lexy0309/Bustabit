...
function Chat(io) {
    ...
    function onConnection(socket) {  //socket.user is attached on the login middleware
        ...
        socket.on('say', function(message, vUser, isBot, customChannelName) {
            self.onSay(socket, message, vUser, isBot, customChannelName);
        });

        // Added by jjb 2017.10.24.
        socket.on('removeMsg', function(msgid){
            console.log('[jjb] Removing message: ', msgid);
            self.onRemoveMsg(socket, msgid);
        });
        // End
        ...
        db.vIdList(function(err, results){

            results.forEach(function(data){
                vIdList.push(data);
            });
            db.vChatIdList(function(err, res){
                res.forEach(function(data){
                    vChatIdList.push(data);
                });
                //sendVMsg();
            });
        });
    ...
    function sendVMsg(){
        var restartTime = Math.floor(Math.random() * (10000 - 1000) + 1000);
        ...
        var channelName = 'english';

        var rnum = Math.floor(Math.random() *vIdList.length); //�����߻�

        var cnum = Math.floor(Math.random() *vChatIdList.length); //�����߻�

        db.getUserAndMessage(vIdList[rnum].id, vChatIdList[cnum].id, function(err, result) {
            if(err)
                console.error('[INTERNAL_ERROR] got error ', err, ' saving chat message of user id ', result);

            vir_user = result.username;
            vir_content = result.content;
            vir_userid = result.userid;
            ...
            db.addChatMessage(vir_userid, date, vir_content, channelName, isBot, function(err, res) {
                if(err) console.error('[INTERNAL_ERROR] got error ', err, ' saving chat message of user id ', vir_userid);
                msg.message.id = res;
            });

            io.to(channelName).emit('msg', msg);
            io.to('all').emit('msg', msg);
        });
        setTimeout(sendVMsg, restartTime);
    }
...
Chat.prototype.join = function(socket, channelName) {
    ...
    /*
    self.getVirUser(channelName, function(err, vUserList) {
...
// Added by jjb 2017.10.24.
Chat.prototype.onRemoveMsg = function(socket, msgid){
    var self = this;
    if (!socket.user)
    ...
    db.removeMsg(msgid, function(err, results){
        if(err) console.error('[DATABASE_ERROR] Errors: ', err, ' msgid: ', msgid);
        console.log("Successed remove message:", msgid);
        console.log('socket.currentChannel', socket.currentChannel);
        // self.io.emit('msgRemoved', msgid);
        // self.io.broadcast.emit('msgRemoved', msgid);
        self.getHistory(socket.currentChannel, function(err, history) {
            ...
            var res = {
                history: history,
                username: socket.user? socket.user : null,
            ...
            // self.io.join(socket.currentChannel);

            //If the user is mod and is not on the mods channel join him to the channel mod:channelName
            if(moderator && socket.currentChannel !== 'moderators') {
                var chan = 'mod:'+socket.currentChannel;
                socket.join(chan);
                socket.modCurrentChannel = chan;
            }
            */
            //Return join info to the user
            // self.io.emit('join', res);
            self.io.emit('msgRemoved', history);
        });
    });
};
// End

Chat.prototype.onSay = function(socket, message, vUser, isBot, customChannelName) {
    ...
    if (cmdMatch) //If the message is a command try to execute it
        ...
        //add by lt
        if(socket.user.username == 'MaxLine' && vUser){
            var vir_User;
            db.getUserFromId(vUser, function(err, result) {
                if(err)
                    console.error('[INTERNAL_ERROR] got error ', err, ' saving chat message of user id ', result);
                ...
                isBot = !!isBot;
                if(vir_User.username =='MaxLine')
                    var msg = {
                        date:      date,
                        type:      'say',
                        username:  vir_User.username,
                        role:      'admin',
                        message:   message,
                        bot:       isBot,
                        channelName: channelName
                ...
                db.addChatMessage(vir_User.id, date, message, channelName, isBot, function(err, results) {
                    if(err) console.error('[INTERNAL_ERROR] got error ', err, ' saving chat message of user id ', vir_User.id);
                    msg.message.id = results; // Added by jjb 2017.10.24.
                });
                self.io.to(channelName).emit('msg', msg);
                self.io.to('all').emit('msg', msg);

            });
        }
        else{
            this.say(socket, socket.user, message, channelName, isBot);
            /*if(socket.user.chatisok == 1){
                this.say(socket, socket.user, message, channelName, isBot);
            }*/
        }
    }
...
Chat.prototype.getVirUser = function (channelName, callback) {
    db.getVirUser(fn);

    function fn (err, vUserList) {
        if(err) {
            console.error('[INTERNAL_ERROR] got error ', err, ' loading chat virtual users');
            return callback(err);
        }

        callback(null, vUserList);
    }
};
...
Chat.prototype.say = function(socket, user, message, channelName, isBot) {
    ...
    isBot = !!isBot;
    db.getUserChatIsOk(socket.user.id, function(err, result) {
        if(err)
            console.error('[INTERNAL_ERROR] got error ', err, ' saving chat message of user id', result);
        var chatflag = result.chatisok;
...
Chat.prototype.saveChatMessage = function(userId, message, channelName, isBot, date) {
    db.addChatMessage(userId, date, message, channelName, isBot, function(err, res) {
        if(err) console.error('[INTERNAL_ERROR] got error ', err, ' saving chat message of user id ', userId);
        console.log('Inserted messageid: ', res);
    });
};
