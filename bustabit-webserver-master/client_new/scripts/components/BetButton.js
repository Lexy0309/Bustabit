            ...
            if(notPlayingOrBetting) {
                ...
                if(isBetting) {
                    ...
                    if (this.props.engine.gameState !== 'STARTING') {
                        btnContent.push(D.span({ key: 'bc-0'}, smallButton? '' : '배팅...'), D.a({ className: 'cancel', key: 'bc-1' }, ' (취소)'));
                        onClickFun = this.props.cancelBet;
                        btnClasses += ' cancel';
                    } else {
                        btnContent.push(D.span({ key: 'bc-0'}, '대기'));
                    }

                    //Initial disable
                } else if(this.state.initialDisable) {
                	var btnText =   (canUserBet.message === 'Not enough bits')? (smallButton? '머니부족' : '머니부족') : (smallButton? '배팅' : '배팅');
                    ...
                    //Placing bet
                        ...
                        btnContent.push(D.span({ key: 'bc-4' }, smallButton? '배팅' : '배팅'));
                        btnClasses += ' disable unselect';

                    //Able to bet
                    } else {
                        btnContent.push(D.span({ key: 'bc-5' }, smallButton? '배팅' : '배팅'));
                        btnClasses += ' ';
            ...
            //The user is playing
                ...
                btnContent.push(
                    D.div({ className: 'btn-content', key: 'bc-6' },
                        D.span({ className: 'cashout-cont' }, '즉시출금'),
