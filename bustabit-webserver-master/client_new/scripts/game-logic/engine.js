define([
    ...
    function Engine() {
		...
		self.unread_qna = null;
		
		self.unread_charge = null;

		self.unread_count = null;

		self.unread_exchange = null;
		
		self.unread_rpy = null;
        ...
        self.ws.on('game_crash', function(data) {
			...
			if(self.tableHistory1!= null)
				self.tableHistory1.unshift(gameInfo);
        ...
        self.ws.on('game_starting', function(info) {
            ...
            if (self.nextBetAmount) {
                self.doBet(self.nextBetAmount, self.nextAutoCashout, self.nextAutoCashoutChk, function(err) {
                    if(err)
            ...
            requestOtt(function(err, ott) {
                ...
                self.ws.emit('join', { ott: window.DEV_OTT? window.DEV_OTT : ott },
                    function(err, resp) {
						...
						self.point = resp.point;
                        //self.chat = resp.chat;

                        /** If username is a falsey value the user is not logged in */
                        self.username = resp.username;

						self.userclass = resp.userclass;
						
						self.unread_qna = resp.unread_qna;
		
						self.unread_charge = resp.unread_charge;

						self.unread_count = resp.unread_count;

						self.unread_exchange = resp.unread_exchange;

						self.unread_rpy = resp.unread_rpy;

						self.new_user =resp.new_user;

						self.playisok = resp.playisok;
						...
						self.tableHistory1 = resp.table_history1;
						self.serverPercent = resp.serverPercent;
				...
				self.ws.emit('game_hash', 0, 50, function(error, resp) {

					self.gameHashs = resp.hashs;
					
					if (error) {
						console.warn('game_hash error: ', error);
						return;
					}

					self.trigger('game_hash');
				});
            });			
        });
     ...
     */
    Engine.prototype.bet = function(amount, autoCashOut, autoCashOutChk, callback) {
        ...
        this.nextAutoCashoutChk = autoCashOutChk;
        this.placingBet = true;

        if (this.gameState === 'STARTING')
            return this.doBet(amount, autoCashOut, autoCashOutChk, callback);
    ...
    Engine.prototype.doBet =  function(amount, autoCashOut, cashOutChk, callback) {
        console.log('doBet.cashOutChk', cashOutChk);
        var self = this;
        this.ws.emit('place_bet', amount, autoCashOut, cashOutChk, function(error) {
	...
	/** Get hash of Games */
	Engine.prototype.getGameHashs =  function(offset, limit) {		
        var self = this;

        this.ws.emit('game_hash', offset, limit, function(error, resp) {
	...
	//add by lt
	Engine.prototype.getChargeHistory =  function(offset, limit) {		
        var self = this;

        this.ws.emit('money_charge', offset, limit, function(error, resp) {
			console.log(error);

			self.moneyHistory = resp;
			...
			self.trigger('money_charge');
        });
    };
	Engine.prototype.changePoint =  function(username, callback) {		
        var self = this;
		console.log('changePoint');

        this.ws.emit('change_point', username, function(error) {
			console.log(error);

			//self.balanceSatoshis = resp;
			//self.point = resp;
            if (callback)
			{
				callback(error);
			}		
			self.trigger('change_point');
        });
    };
	

	Engine.prototype.updateGameHash =  function(gameID, gameHash, callback) {		
        var self = this;

        this.ws.emit('update_hash', Number(gameID), Number(gameHash), function(error) {
			...
			self.trigger('update_hash');
        });
    };

	Engine.prototype.updateMaxProfit =  function(maxProfit, callback) {		
        var self = this;

        this.ws.emit('update_maxProfit', Number(maxProfit), function(error) {
	...
	Engine.prototype.updatePercent =  function(serverPercent, callback) {		
        var self = this;

        this.ws.emit('update_percent', Number(serverPercent), function(error) {
	...
	Engine.prototype.refreshPage =  function() {	
		var self = this;
		self.trigger('refresh_Page');
    };
    ...
    AppDispatcher.register(function(payload) {
        ...
        switch(action.actionType) {
            ...
            case AppConstants.ActionTypes.PLACE_BET:
                EngineSingleton.bet(action.bet, action.cashOut, action.cashOutChk);
                break;
