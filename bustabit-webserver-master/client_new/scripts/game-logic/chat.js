define([
   'socketio',
    ...
    function Chat() {
        ...
        self.ws.on('msgRemoved', function(data) {
            console.log('From server: removed message: ', data);
            self.username = self.username;
            self.history = data;
            self.moderator = self.moderator;
            self.state = 'MSGREMOVED';
            //self.state = 'JOINED';
            self.trigger('msgRemoved');
        });
        // End
		...
		/*
		self.ws.on('vUserList', function(info) {
            self.vUserList = info.vUserList;
			console.log('self.vUserList=', self.vUserList);
     ...
     */
    Chat.prototype.say = function(msg, vUser, isBot) {
        console.assert(msg.length >= 1 && msg.length < 500);
		this.ws.emit('say', msg, vUser, isBot);
    };
    // Added by jjb 2017.10.24.
    Chat.prototype.removeMsg  = function(msgid){
      console.log('Emitted removeMsg', msgid);
      this.ws.emit('removeMsg', msgid);
    };
    // End
    ...
    AppDispatcher.register(function(payload) {
        ...
        switch(action.actionType) {
            ...
            case AppConstants.ActionTypes.SAY_CHAT:
                ChatSingleton.say(action.msg, action.vUser);
                break;

            case AppConstants.ActionTypes.REMOVE_MSG:
                ChatSingleton.removeMsg(action.msgid);
                break;
